...
class JsonConfigurator:
    ...
    def customize_json(self) -> str:
        ...
        for setting_key, setting_value in configuration.items():
            if setting_key in self.__CONFIG_KEYS and setting_value:
                ...
                self.__invoices_info = self._convert_key(setting_key, setting_value)
        return json.dumps(self.__invoices_info)

    def _process_keyword(self, setting_key: str):
        return {
    ...
    def _change_value_for_key(self, dict_element, value_to_remove, replacing_value, keys):
    ...
    def replace_value_in_dict(self, dict_element, keys, new_element, replacing_value, value_to_remove):
    ...
    def _convert_key(self, old_key, key_config):
        new_key = key_config['value']
        included = key_config['included']
        invoices_info = self._replace_keys(self.__invoices_info, old_key, new_key, included)
        return invoices_info

    def _replace_keys(self, dict_element: json, old_key: str, new_key: str, included: bool) -> dict:
        if isinstance(dict_element, (str, int, float)):
            return dict_element
        if isinstance(dict_element, dict):
            new_element = dict_element.__class__()
            for k, v in dict_element.items():
                self.remove_element_or_switch_keys(included, k, new_element, new_key, old_key, v)
        elif isinstance(dict_element, (list, set, tuple)):
            new_element = dict_element.__class__(
                self._replace_keys(v, old_key, new_key, included) for v in dict_element)
        else:
            return dict_element
        return new_element

    def remove_element_or_switch_keys(self, included, k, new_element, new_key, old_key, v):
        if k == old_key and included:
            new_element[new_key] = self._replace_keys(v, old_key, new_key, included)
        elif k != old_key:
            new_element[k] = self._replace_keys(v, old_key, new_key, included)
